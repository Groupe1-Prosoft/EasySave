...
flowchart LR
 subgraph S["EasySave 1.0 (Console)"]
    ...
    UC_Exec -. "«extend»<br/>[type = Full]" .-> UC_Full
    UC_Exec -. "«extend»<br/>[type = Differential]" .-> UC_Diff
...
sequenceDiagram
    ...
    ConsoleView->>User: Request job details (name, source, target, type)
    User->>ConsoleView: Provide details

    ConsoleView->>BackupService: CreateJob(name, source, target, type)
    BackupService->>Configuration: GetJobs()
    Configuration-->>BackupService: List of current jobs

    alt Less than 5 jobs
        BackupService->>BackupJob: new BackupJob(details)
        BackupJob->>BackupJob: Validate()
        BackupJob-->>BackupService: Valid job

        BackupService->>Configuration: AddJob(job)
...
sequenceDiagram
    ...
    ConsoleView->>BackupService: ExecuteJob(job)

    BackupService->>BackupState: new BackupState(jobName)
    BackupService->>BackupState: UpdateState(ACTIVE)

    loop For each file in source
        BackupService->>BackupService: CopyFile(source, dest)

        BackupService->>LogData: new LogData(fileInfo, transferTime)
        BackupService->>ILogger: WriteLog(logData)

        BackupService->>BackupState: UpdateProgress(filesRemaining, sizeRemaining)
...
sequenceDiagram
    ...
    ConsoleView->>BackupService: DeleteJob(3)
    BackupService->>Configuration: RemoveJob(3)
    Configuration->>Configuration: SaveConfig()
...
sequenceDiagram
    ...
    User->>ConsoleView: Select "English"

    ConsoleView->>LanguageManager: SetLanguage("EN")
...
sequenceDiagram
    ...
    loop For each file in source
        BackupService->>BackupService: IsFileModified(file)
